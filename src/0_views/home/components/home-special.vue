  ...
  <HomePanel title="最新专题">
    ...
    <div class="special-list" ref="target">
      <div class="special-item" v-for="item in specialList" :key="item.id">
        ...
        <div class="foot">
          <span class="like"
            ><i class="iconfont icon-kongxin"></i>{{ item.collectNum }}</span
          >
          <span class="view"
            ><i class="iconfont icon-guankan01"></i>{{ item.viewNum }}</span
          >
          <span class="reply"
            ><i class="iconfont icon-liuyan"></i>{{ item.replyNum }}</span
          >
        </div>
      </div>
    </div>
...
const lyData = useLazyData(findSpecial);
const target = lyData.target;
const specialList = lyData.result;
</script>
